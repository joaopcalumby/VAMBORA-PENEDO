...
def _serialize_payment_info(payment_methods: Optional[List[str]]) -> str:
    ...
    return ", ".join(methods) if methods else "Dinheiro"


def _parse_payment_info(raw_payment_info: Optional[str]) -> List[str]:
    if not raw_payment_info:
        return ["Dinheiro"]

    methods = [m.strip() for m in raw_payment_info.split(",") if m and m.strip()]
    return methods if methods else ["Dinheiro"]
